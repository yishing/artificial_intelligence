  ...
  def elapseTime(self, gameState):
    ...
    for position in self.legalPositions:
        if self.beliefs[position]>0:
          ...
          for pos,por in newPosDist.items():
            allPossible[pos]=self.beliefs[position]*por+allPossible[pos]
    # allPossible.normalize()
    self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    for i in range(self.numParticles):
      newList.append(random.choice(self.legalPositions))
  ...
  def observe(self, observation, gameState):
    ...
    if(noisyDistance==None):
      ...
      for particle in self.particles:
        weightedParticles[particle]=weightedParticles[particle]+emissionModel[util.manhattanDistance(particle, pacmanPosition)]
      ...
      if weightedParticles.totalCount() == 0:
        ...
        newP=[]
        for i in range(self.numParticles):
          newP.append(util.sample(weightedParticles))
        self.particles=newP
        return
...
class JointParticleFilter:
  ...
  def observeState(self, gameState):
    ...
    for  i in range(self.numGhosts):
      if(noisyDistances[i]==None):
        ...
        for particle in self.particles:
          ...
          updatedparticle=tuple(particleList)
    ...
    for particle in self.particles:
      ...
      for i in range(self.numGhosts):
        if noisyDistances[i] is not None:
          belief=belief*emissionModels[i][util.manhattanDistance(particle[i],pacmanPosition)]
      weightedCounter[particle]=weightedCounter[particle]+belief
    if weightedCounter.totalCount()==0:
      ...
      updatedParticles=[]
      for i in range(self.numParticles):
        updatedParticles.append(util.sample(weightedCounter))
