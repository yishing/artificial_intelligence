...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    possiblePos=None
    possibleDis=float("inf")
    for livingGhostPositionDistribution in livingGhostPositionDistributions:
      mostPossible=livingGhostPositionDistribution.argMax()
      dist=self.distancer.getDistance(pacmanPosition,mostPossible)
      if dist<possibleDis:
        possibleDis=dist
        possiblePos=mostPossible
    bestAction=None
    closestDis=float("inf")
    for action in legal:
      newpacmanPosition=Actions.getSuccessor(pacmanPosition,action)
      newDis=self.distancer.getDistance(newpacmanPosition,possiblePos)
